...
import com.daking.app.client_common.view.listview.pulltorefresh.PullToRefreshListFooter;
import com.daking.app.client_common.view.listview.pulltorefresh.PullToRefreshListHeader;
import com.daking.app.client_common.view.listview.swipemenu.SwipeMenu;
import com.daking.app.client_common.view.listview.swipemenu.SwipeMenuAdapter;
import com.daking.app.client_common.view.listview.swipemenu.SwipeMenuCreator;
import com.daking.app.client_common.view.listview.swipemenu.SwipeMenuLayout;
import com.daking.app.client_common.view.listview.swipemenu.SwipeMenuView;

import com.daking.app.client_common.R;

/**
 * 下拉刷新/上拉加载更多/侧滑的listview
 */
public class PullToRefreshSwipeMenuListView extends ListView implements OnScrollListener {
    ...
    private void init(Context context) {
        ...
        MAX_X = DisplayUtil.dip2px(MAX_X);
        MAX_Y = DisplayUtil.dip2px(MAX_Y);
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                // 1.记录开始按下X和开始按下Y;
                // 2.记录此次Y;
                // 3.当按下的item是上次打开侧滑菜单的item时,状态设为"左右滑动",将触摸事件委托给SwipeMenuLayout的onSwipe();并返回true;
                // 4.当上一次触摸的item的侧滑菜单处于open时,平滑关闭其菜单,并返回;
                // 5.记录按下item,将触摸事件委托给SwipeMenuLayout的onSwipe();
                mLastY = ev.getRawY();
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                // 1.此次Y与上次Y的差值;
                // 2.记录此次Y;
                // 3.dy = 此次Y与开始按下Y的差的绝对值; dx = 此次X与开始按下X的差的绝对值;
                // 4.没有侧滑菜单被移动 且 dx少于dy的3倍,视为垂直滑动;
                // 5.若当前状态为"左右滑动",将触摸事件委托给SwipeMenuLayout的onSwipe();执行ACTION_CANCEL操作,最后返回true;
                // 6.dy大于指定Y值时,状态变成"上下滑动"; dx大于指定X值时,状态变成"左右滑动";
            ...
            case MotionEvent.ACTION_UP:
                // 鼠标弹起:
                // 1.判断尾部是否被拉伸到满足"加载更多"的长度;
                // 2.判断首个item是否可见且头部被拉伸到满足"刷新"的长度;
                // 3.若当前状态为"左右滑动",将触摸事件委托给SwipeMenuLayout的onSwipe();执行ACTION_CANCEL操作,最后返回true;
                mLastY = -1; // reset
